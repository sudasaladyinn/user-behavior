...
### 二、分析目的
通过对用户行为数据进行统计、分析，揭示用户使用产品的规律，从而发现产品功能、营销策略以及运营策略存在的问题，实现更精准和精细的营销与运营，提高用户购买体验、助力商家获利同时实现平台业务增长。
...
#### 2.字段含义
<img src="./pic/a.png" width = "698" height = "136" alt="图片名称" align=center />
其中用户行为类型有四种：
<img src="./pic/b.png" width = "760" height = "110" alt="图片名称" align=center />

数据集规模如下：
<img src="./pic/c.png" width = "650" height = "106" alt="图片名称" align=center />
### 四、分析思路
<img src="./pic/d.png" width = "650" height = "480" alt="图片名称" align=center />
提出问题：

	1.最受欢迎的产品/类目，优化产品结构

	2.了解用户活跃的日期以及每天活跃的时间段

	3.建立RFM模型分析用户行为，计算R值、F值、M值，根据价值打分，将用户行为分类 ，针对不同价值的用户提出不同的建议

	4.用户从浏览到最终购买整个过程的流失情况，从而有目的性的进行统计

	5.产品基础指标：pv、uv等；流量质量指标：平均购买次数、用户留存率、复购率等
### 五、数据处理
（一）准备数据
使用Navicat导入功能，将数据导入，如图：
<img src="./pic/e.png" width = "650" height = "366" alt="图片名称" align=center />
<img src="./pic/f.png" width = "650" height = "366" alt="图片名称" align=center />
...
<img src="./pic/g.png" width = "350" height = "266" alt="图片名称" align=center />

2.删除重复值

	select * from UserBehavior 
	group by user_id,item_id,cat_id,behavior_type,time_stamp
	having count(*) > 1;
	
结果如下：

<img src="./pic/h.png" width = "650" height = "80" alt="图片名称" align=center />
结果表明所选数据集中不含重复值。
...
<img src="./pic/j.png" width = "750" height = "250" alt="图片名称" align=center />
...
<img src="./pic/k.png" width = "850" height = "330" alt="图片名称" align=center />
...
数据集采集的用户行为数据时间在2017年11月25日至2017年12月3日之间，因此不在此区间的异常值需要剔除。
	...
	where `date` in(‘2017-09-11’,’2017-11-24’,’2017-11-20’,’2017-11-23’,
	’2017-11-17’,’2017-11-19’,’2017-11-22’,’2017-11-14’,’2017-11-21’,
	’2017-11-16’,’2017-11-18’,’2017-11-10’,’2017-12-04’,’2017-11-06’,
...
<img src="./pic/l.png" width = "850" height = "45" alt="图片名称" align=center />
...
<img src="./pic/m.png" width = "850" height = "210" alt="图片名称" align=center />
...
<img src="./pic/n.png" width = "850" height = "190" alt="图片名称" align=center />
...
<img src="./pic/o.png" width = "850" height = "230" alt="图片名称" align=center />

总体来说用户日均活跃度大致平稳，但在12月2日和12月3日出现小幅增长，其中点击数、加购数、收藏数增加明显，但购买量增幅放缓，经分析，考虑到十二月份节日活动较多，部分商家选择在月初进行活动预热，用户偏向于提前收藏、加入购物车，符合预期习惯。

<img src="./pic/p.png" width = "850" height = "250" alt="图片名称" align=center />

用户活跃度在凌晨到4点之间骤减，5点到10点攀升，10点到18点趋于平稳，18点到22点急剧攀升并在22点达到峰值，随后下降。符合用户作息时间规律，可以考虑在20-23点之间上架新品，或者借机举行促销活动以此来提高商品转化率。

3.建立RFM模型分析用户行为

RFM模型是衡量客户价值和客户创利能力的重要工具。RFM分析方法通过最近一次消费时间间隔（Recently）、消费频率（frequency）、消费金额（money）三个指标对用户进行分类。

**最近一次消费消时间间隔：**反映客户对产品的态度以及品牌价值的信任度，关乎用户的留存状况。用户最近一次消费时间距离今日时间越近，当前的活跃度将会越高，用户价值也越高。本数据集选取的时间跨度为2017年11月25日至2017年12月3日之间，因此选择用户最近一次消费距2017年12月3日的间隔。最近一次消费是最有力的预测指标。由于本数据集缺少消费金额相关数据，故使用消费时间间隔、消费频率两个指标进行分类。
...
<img src="./pic/q.png" width = "750" height = "130" alt="图片名称" align=center />
...
结果如下：
<img src="./pic/r.png" width = "750" height = "170" alt="图片名称" align=center />

3）用户分类

用户R、F得分大于平值，标签记为'高'，否则记为'低'，用户分类标准如下：

<img src="./pic/s.png" width = "700" height = "150" alt="图片名称" align=center />
...
<img src="./pic/t.png" width = "550" height = "150" alt="图片名称" align=center />
...
用户行为情况：
	...
	-- 用户行为统计
	select behavior_type as '用户行为', count(behavior_type) as '行为数量'
	from UserBehavior
	group by behavior_type
	order by count(behavior_type) desc;
	
<img src="./pic/u.png" width = "650" height = "350" alt="图片名称" align=center />

从浏览到最终的购买支付转化率仅为2%，由此可见用户在操作的环节过程中存在大量的流失情况，用户具体在哪个环节流失，需要进一步分析。用户有四种行为，分别是：点击、收藏、加购和购买。用户在购买过程中主要存在以下三种行为路径：
...
<img src="./pic/v.png" width = "650" height = "150" alt="图片名称" align=center />
...
<img src="./pic/x.png" width = "850" height = "250" alt="图片名称" align=center />
...
<img src="./pic/aa.png" width = "850" height = "260" alt="图片名称" align=center />
...
<img src="./pic/dd.png" width = "750" height = "320" alt="图片名称" align=center />

收藏后购买的转化率为0.1%

<img src="./pic/ee.png" width = "750" height = "260" alt="图片名称" align=center />
...
+ 加购后购买的转化率为0.3%，而收藏后购买的转化率仅为0.1%，相比购物车来说收藏功能的转化率很低，针对收藏功能可以对消费者进行问卷调查，看用户是否对收藏感兴趣，以及是否是用户习惯导致用户更喜欢加入购物车。
...
<img src="./pic/ff.png" width = "750" height = "220" alt="图片名称" align=center />

<img src="./pic/gg.png" width = "750" height = "290" alt="图片名称" align=center />

+ 上图看出12月2日访问量与日活跃用户数出现出现大幅增长，日pv较前日增长26%，日uv较前日增长了31%。
+ 人均页面访问次数稳定在13左右。

统计在各种行为下的操作数、用户人数以及人均操作数：

<img src="./pic/hh.png" width = "730" height = "120" alt="图片名称" align=center />

<img src="./pic/ii.png" width = "730" height = "320" alt="图片名称" align=center />

**低收藏:** 收藏的用户数仅占所有用户的40%，漏斗模型中已经对收藏-支付转化进行详细分析。

**高浏览量：**在9天时间中，平均每位用户浏览商品详情页总计92次，人均购买数为3次，就是说用户每浏览30次左右才会产生一次购买行为，购买转化率较低。应该是否是商品推送不精准导致用户无法找到心仪的商品，从而使得用户最终购买率降低。

**高付费率：**产生购买行为的用户占69%，人均购买3次，表明用户的付费比例高，用户对平台的依赖性大
...
<img src="./pic/jj.png" width = "730" height = "210" alt="图片名称" align=center />
...
<img src="./pic/ll.png" width = "730" height = "190" alt="图片名称" align=center />

<img src="./pic/mm.png" width = "630" height = "260" alt="图片名称" align=center />
...
3）用户活跃度在白天活跃时间段在10点到18点，黄金时间段在18点到23点之间，于22点达到一天活跃量峰值，这个时间段用户成交量最高。
...
3）建议商家对用户的行为轨迹进行分析，如根据用户的收藏加购以及搜索记录，确定用户的品味，同时根据浏览记录以及浏览时长细化用户需求，对用户画像进行分析，分析用户的潜在需求，个性化推荐商品，以降低跳出率，提高用户活跃度。
